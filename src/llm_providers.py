...
class LLMClient:
    ...
    async def generate(
        ...
        async def _attempt_call(m_key: str, m_config: ModelConfig) -> LLMResponse:
            if m_config.provider == LLMProvider.OPENAI:
                return await self._call_openai(full_prompt, system_prompt, m_config)
            elif m_config.provider == LLMProvider.ANTHROPIC:
                return await self._call_anthropic(full_prompt, system_prompt, m_config)
            elif m_config.provider == LLMProvider.GEMINI:
                return await self._call_gemini(full_prompt, system_prompt, m_config)
            elif m_config.provider == LLMProvider.GROQ:
                return await self._call_groq(full_prompt, system_prompt, m_config)
            elif m_config.provider == LLMProvider.OLLAMA:
                return await self._call_ollama(full_prompt, system_prompt, m_config)
            else:
